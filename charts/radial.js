...
function drawRadial(data) {
  ...
  // Red → green based on pass rate
  const colorScale = d3.scaleSequential()
    .domain([0.25, 0.75])
    .interpolator(d3.interpolateRgb("#C9503A", "#2D6A4F"))
    .clamp(true);

  // ---- Circular gridlines at 25 / 50 / 75 % ----
  ...
  // ---- Background arcs (full length, light fill) ----
  const bgArc = d3.arc()
    .innerRadius(innerR)
    .outerRadius(outerR)
